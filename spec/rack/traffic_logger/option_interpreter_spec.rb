      ...
      VERBS.each do |verb|
        define_method verb do |*args|
          ...
          @context.it message do
            case args.first
              when true, false
                expect(subject.request? verb, code).to be args.first
                expect(subject.response? verb, code).to be args.first
                FLAGS.each { |flag| expect(subject.__send__ :"#{flag}?", verb, code).to be false }
              when :all
                expect(subject.request? verb, code).to be true
                expect(subject.response? verb, code).to be true
